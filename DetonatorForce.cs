...
public class DetonatorForce : DetonatorComponent
{
	...
	public override void Explode()
	{
		...
		if (_explodeDelay <= 0f)
		{
			...
			{
				if (!collider || !((Component)collider).get_rigidbody())
				{
					continue;
				}
				((Component)collider).get_rigidbody().AddExplosionForce(power * size, _explosionPosition, radius * size, 4f * MyDetonator().upwardsBias * size);
				...
				if ((bool)fireObject)
				{
					...
					_tempFireObject = (Object.Instantiate((Object)fireObject, base.transform.position, base.transform.rotation) as GameObject);
					_tempFireObject.transform.parent = collider.transform;
					_tempFireObject.transform.localPosition = new Vector3(0f, 0f, 0f);
					if ((bool)_tempFireObject.get_particleEmitter())
					{
						_tempFireObject.get_particleEmitter().emit = true;
