...
internal class NetworkingPeer : LoadbalancingPeer, IPhotonPeerListener
{
	...
	public override void Disconnect()
	{
		if (base.PeerState == PeerStateValue.Disconnected)
		{
			...
			{
				DebugReturn(DebugLevel.WARNING, $"Can't execute Disconnect() while not connected. Nothing changed. State: {State}");
			}
		}
		else
		{
	...
	private void HandleEventLeave(int actorID)
	{
		...
		if (actorID < 0 || !mActors.ContainsKey(actorID))
		{
			...
			{
				DebugReturn(DebugLevel.ERROR, $"Received event Leave for unknown actorNumber: {actorID}");
			}
			return;
		}
	...
	public bool OpJoin(string gameID)
	{
		...
		bool flag = State == global::PeerState.Joining;
		return OpJoinRoom(gameID, (!flag) ? null : GetLocalActorProperties());
	}
	...
	public void OnOperationResponse(OperationResponse operationResponse)
	{
		...
		case 230:
			if (operationResponse.ReturnCode != 0)
			{
				...
				{
					DebugReturn(DebugLevel.ERROR, $"If you host Photon yourself, make sure to start the 'Instance LoadBalancing'");
				}
				else if (operationResponse.ReturnCode == short.MaxValue)
				{
					DebugReturn(DebugLevel.ERROR, $"The appId this client sent is unknown on the server (Cloud). Check settings. If using the Cloud, check account.");
				}
				else if (operationResponse.ReturnCode == 32755)
				{
					DebugReturn(DebugLevel.ERROR, $"Custom Authentication failed (either due to user-input or configuration or AuthParameter string format). Calling: OnCustomAuthenticationFailed()");
				...
				{
					DebugReturn(DebugLevel.ERROR, $"Authentication failed: '{operationResponse.DebugMessage}' Code: {operationResponse.ReturnCode}");
				}
				Disconnect();
				State = global::PeerState.Disconnecting;
				if (operationResponse.ReturnCode == 32757)
				{
					DebugReturn(DebugLevel.ERROR, $"Currently, the limit of users is reached for this title. Try again later. Disconnecting");
				...
				{
					DebugReturn(DebugLevel.ERROR, $"The used master server address is not available with the subscription currently used. Got to Photon Cloud Dashboard or change URL. Disconnecting");
		...
		case 227:
			if (State != global::PeerState.Joining)
			{
				if (operationResponse.ReturnCode != 0)
				{
					if ((int)base.DebugOut >= 1)
					{
						DebugReturn(DebugLevel.ERROR, $"createGame failed, client stays on masterserver: {operationResponse.ToStringFull()}.");
					}
					SendMonoMessage(PhotonNetworkingMessage.OnPhotonCreateRoomFailed);
					break;
				}
		...
		case 226:
			if (State != global::PeerState.Joining)
			{
				if (operationResponse.ReturnCode != 0)
				{
					...
					if ((int)base.DebugOut >= 2)
					{
						DebugReturn(DebugLevel.WARNING, $"JoinRoom failed (room maybe closed by now). Client stays on masterserver: {operationResponse.ToStringFull()}. State: {State}");
					}
				}
				else
				{
		...
		case 225:
			if (operationResponse.ReturnCode != 0)
			{
				...
				{
					DebugReturn(DebugLevel.WARNING, "JoinRandom failed: No open game. Client stays in lobby.");
				}
				else if ((int)base.DebugOut >= 1)
				{
					DebugReturn(DebugLevel.ERROR, $"JoinRandom failed: {operationResponse.ToStringFull()}.");
				}
				SendMonoMessage(PhotonNetworkingMessage.OnPhotonRandomJoinFailed);
			}
			else
		...
		default:
			if ((int)base.DebugOut >= 1)
			{
				DebugReturn(DebugLevel.ERROR, $"operationResponse unhandled: {operationResponse.ToString()}");
			}
			break;
		case 252:
		case 253:
	...
	public void OnStatusChanged(StatusCode statusCode)
	{
		if ((int)base.DebugOut >= 3)
		{
			DebugReturn(DebugLevel.INFO, $"OnStatusChanged: {statusCode.ToString()}");
		}
		switch (statusCode)
		{
	...
	public void OnEvent(EventData photonEvent)
	{
		if ((int)base.DebugOut >= 3)
		{
			DebugReturn(DebugLevel.INFO, $"OnEvent: {photonEvent.ToString()}");
		}
		int num = -1;
			...
			if (mActors[num] == mLocalActor)
			{
				...
				foreach (int num3 in array2)
				{
					if (mLocalActor.ID != num3 && !mActors.ContainsKey(num3))
					{
						...
						AddNewPlayer(num3, new PhotonPlayer(isLocal: false, num3, string.Empty));
					}
				}
		...
		case 206:
		{
			...
			for (short num7 = num6; num7 < hashtable4.Count; num7 = (short)(num7 + 1))
			{
		...
		case 207:
		{
			...
			if (num4 >= 0)
			{
				DestroyPlayerObjects(num4, localOnly: true);
				break;
			}
		...
		case 204:
		{
			...
			Debug.Log("Ev Destroy for viewId: " + num2 + " sent by owner: " + (num2 / PhotonNetwork.MAX_VIEW_IDS == num) + " this client is owner: " + (num2 / PhotonNetwork.MAX_VIEW_IDS == mLocalActor.ID));
	...
	internal ExitGames.Client.Photon.Hashtable SendInstantiate(string prefabName, Vector3 position, Quaternion rotation, int group, int[] viewIDs, object[] data, bool isGlobalObject)
	{
		...
		OpRaiseEvent(202, hashtable, sendReliable: true, 0, cache, ReceiverGroup.Others);
		return hashtable;
	}
	...
	internal GameObject DoInstantiate(ExitGames.Client.Photon.Hashtable evData, PhotonPlayer photonPlayer, GameObject resourceGameObject)
	{
		...
		GameObject gameObject = (GameObject)UnityEngine.Object.Instantiate((UnityEngine.Object)resourceGameObject, position, rotation);
	...
	private void ServerCleanInstantiateAndDestroy(int instantiateId, int actorNr)
	{
		...
		OpRaiseEvent(202, hashtable, sendReliable: true, 0, new int[1]
		{
			actorNr
		}, EventCaching.RemoveFromRoomCache);
		ExitGames.Client.Photon.Hashtable hashtable2 = new ExitGames.Client.Photon.Hashtable();
		hashtable2[(byte)0] = instantiateId;
		OpRaiseEvent(204, hashtable2, sendReliable: true, 0, EventCaching.DoNotCache, ReceiverGroup.Others);
	}
	...
	private void SendDestroyOfPlayer(int actorNr)
	{
		...
		OpRaiseEvent(207, hashtable, sendReliable: true, 0, EventCaching.DoNotCache, ReceiverGroup.Others);
	}
	...
	{
		OpRaiseEvent(202, null, sendReliable: true, 0, new int[1]
		{
			actorNr
		}, EventCaching.RemoveFromRoomCache);
	}
	...
	{
		OpRaiseEvent(200, null, sendReliable: true, 0, new int[1]
		{
			actorNumber
		}, EventCaching.RemoveFromRoomCache);
	}
	...
	{
		OpRaiseEvent(0, null, sendReliable: true, 0, new int[1]
		{
			actorNumber
		}, EventCaching.RemoveFromRoomCache);
	}
	...
	{
		OpRaiseEvent(0, null, sendReliable: true, 0, EventCaching.RemoveFromRoomCache, ReceiverGroup.MasterClient);
	}
	...
	public void OpCleanRpcBuffer(PhotonView view)
	{
		...
		OpRaiseEvent(200, hashtable, sendReliable: true, 0, EventCaching.RemoveFromRoomCache, ReceiverGroup.Others);
	}
	...
	internal void RPC(PhotonView view, string methodName, PhotonPlayer player, params object[] parameters)
	{
		if (!blockSendingGroups.Contains(view.group))
		{
			...
			OpRaiseEvent(200, hashtable, sendReliable: true, 0, targetActors);
		}
	}
	...
	internal void RPC(PhotonView view, string methodName, PhotonTargets target, params object[] parameters)
	{
		...
		case PhotonTargets.All:
			OpRaiseEvent(200, (byte)view.group, hashtable, sendReliable: true, 0);
		...
		case PhotonTargets.AllBuffered:
			OpRaiseEvent(200, hashtable, sendReliable: true, 0, EventCaching.AddToRoomCache, ReceiverGroup.Others);
			ExecuteRPC(hashtable, mLocalActor);
			break;
		...
		case PhotonTargets.MasterClient:
			...
			{
				OpRaiseEvent(200, hashtable, sendReliable: true, 0, EventCaching.DoNotCache, ReceiverGroup.MasterClient);
			}
			break;
		default:
	...
	public void RunViewUpdate()
	{
		...
		{
			OpRaiseEvent(206, (byte)item.Key, item.Value, sendReliable: true, 0);
		}
		foreach (KeyValuePair<int, ExitGames.Client.Photon.Hashtable> item2 in dictionary2)
		{
			OpRaiseEvent(201, (byte)item2.Key, item2.Value, sendReliable: false, 0);
		}
	}
