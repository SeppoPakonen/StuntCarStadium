...
public class TasharenWater : MonoBehaviour
{
	...
	private Camera GetReflectionCamera(Camera current, Material mat, int textureSize)
	{
		if (!mTex || mTexSize != textureSize)
		{
			...
			mTex.hideFlags = HideFlags.DontSaveInEditor;
	...
	{
		if (keepUnderCamera && Camera.main != null && Camera.main.transform != null)
		{
	...
	private void OnWillRenderObject()
	{
		...
		if ((int)reflectionMask == 0 || reflectionTextureSize < 512)
		{
		...
		mIsRendering = true;
		Camera reflectionCamera = GetReflectionCamera(current, sharedMaterial, reflectionTextureSize);
