...
public static class PhotonNetwork
{
	...
	public static bool SetMasterClient(PhotonPlayer player)
	{
		...
		return networkingPeer.SetMasterClient(player.ID, sync: true);
	}
	...
	public static void Connect(string serverAddress, int port, string appID, string gameVersion)
	{
		...
		networkingPeer.mAppVersion = gameVersion + "_1.22.1";
	...
	{
		CreateRoom(roomName, isVisible: true, isOpen: true, 0, null, null);
	}
	...
	public static GameObject Instantiate(string prefabName, Vector3 position, Quaternion rotation, int group, object[] data)
	{
		...
		Component[] componentsInChildren = value.GetComponentsInChildren<PhotonView>(includeInactive: true);
	...
	public static void CloseConnection(PhotonPlayer kickPlayer)
	{
		if (VerifyCanUseNetwork())
		{
			...
			networkingPeer.OpRaiseEvent(203, null, sendReliable: true, 0, targetActors);
		}
	}
	...
	public static void Destroy(PhotonView targetView)
	{
		if (targetView != null)
		{
			networkingPeer.RemoveInstantiatedGO(targetView.gameObject, localOnly: false);
		}
		else
		{
	...
	{
		networkingPeer.RemoveInstantiatedGO(targetGo, localOnly: false);
	}
