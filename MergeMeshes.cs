...
public class MergeMeshes : MonoBehaviour
{
	...
	public void Merge(bool immediate)
	{
		...
		foreach (MeshFilter meshFilter in array)
		{
			...
			if (material == null)
			{
				material = ((Component)meshFilter).get_renderer().sharedMaterial;
			}
		...
		foreach (MeshFilter meshFilter2 in array9)
		{
			...
			Renderer renderer = ((Component)meshFilter2).get_renderer();
			...
			if (afterMerging == PostMerge.DisableGameObjects)
			{
				...
				while (transform != mTrans)
				{
					if (((Component)transform).get_rigidbody() != null)
					{
				...
				TWTools.SetActive(gameObject2, state: false);
			}
			...
			{
				ref Vector3 reference = ref array2[num10];
				reference = worldToLocalMatrix.MultiplyPoint3x4(localToWorldMatrix.MultiplyPoint3x4(vertices[k]));
				if (array6 != null)
				{
					ref Vector3 reference2 = ref array6[num10];
					reference2 = worldToLocalMatrix.MultiplyVector(localToWorldMatrix.MultiplyVector(array10[k]));
				}
				if (array8 != null)
				{
					ref Color reference3 = ref array8[num10];
					reference3 = array14[k];
				}
				if (array4 != null)
				{
					ref Vector2 reference4 = ref array4[num10];
					reference4 = array12[k];
				}
				if (array5 != null)
				{
					ref Vector2 reference5 = ref array5[num10];
					reference5 = array13[k];
				}
				if (array7 != null)
				{
		...
		if (array2.Length > 0)
		{
			...
			{
				mMesh = new Mesh();
				mMesh.hideFlags = HideFlags.DontSaveInEditor;
			}
			else
			{
				mMesh.Clear();
	...
	public void Clear()
	{
		...
		for (int count = mDisabledGO.Count; i < count; i++)
		{
			...
			if ((bool)gameObject)
			{
				TWTools.SetActive(gameObject, state: true);
			}
		}
		int j = 0;
