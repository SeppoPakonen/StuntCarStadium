...
public class Loader : GuiClasses
{
	...
	{
		Debug.Log("[VERBOSE] Loader Awake - Starting initialization");
		System.Console.WriteLine("[VERBOSE] Loader Awake - Starting initialization");
		if (UnityEngine.Object.FindObjectsOfType(typeof(Loader)).Length > 1)
		{
			Debug.LogWarning("[VERBOSE] Duplicate Loader found, destroying this instance");
			System.Console.WriteLine("[VERBOSE] Duplicate Loader found, destroying this instance");
			UnityEngine.Object.Destroy(base.gameObject);
			return;
		}
		try
		{
			Debug.Log("[VERBOSE] Testing PlayerPrefsObscured storage");
			System.Console.WriteLine("[VERBOSE] Testing PlayerPrefsObscured storage");
			PlayerPrefsObscured.SetString("temp", new string('A', 1024));
		}
		catch (Exception ex)
		{
			Debug.LogError("[VERBOSE] PlayerPrefsObscured overload error: " + ex.Message);
			System.Console.WriteLine("[VERBOSE] PlayerPrefsObscured overload error: " + ex.Message);
			bs.LogEvent2("Overload");
			Debug.LogError("Overload");
			PlayerPrefsObscured.DeleteAll();
		}
		PlayerPrefsObscured.DeleteKey("temp");
		Debug.Log("[VERBOSE] Loader Awake - Basic initialization completed");
		System.Console.WriteLine("[VERBOSE] Loader Awake - Basic initialization completed");
		...
		Debug.Log("[VERBOSE] Previous version: " + lastVersion + ", Current version: " + bs.setting.version);
		System.Console.WriteLine("[VERBOSE] Previous version: " + lastVersion + ", Current version: " + bs.setting.version);
		PlayerPrefs.SetInt("lastVersion", bs.setting.version);
		UpdateGlobals();
		Debug.Log("[VERBOSE] UpdateGlobals completed");
		System.Console.WriteLine("[VERBOSE] UpdateGlobals completed");
		bs.setting.InitSettings();
		Debug.Log("[VERBOSE] Settings initialized");
		System.Console.WriteLine("[VERBOSE] Settings initialized");
		UpdateGlobals();
		gameStartTime = Time.realtimeSinceStartup;
		Debug.Log("[VERBOSE] Game start time set: " + gameStartTime);
		System.Console.WriteLine("[VERBOSE] Game start time set: " + gameStartTime);
		UnityEngine.Object.DontDestroyOnLoad(base.gameObject);
		Debug.Log("[VERBOSE] Loader object set to persist across scenes");
		System.Console.WriteLine("[VERBOSE] Loader object set to persist across scenes");
		GA.SettingsGA.Build = bs.setting.version.ToString();
		Debug.Log("[VERBOSE] GameAnalytics build version set: " + bs.setting.version);
		System.Console.WriteLine("[VERBOSE] GameAnalytics build version set: " + bs.setting.version);
		if (Resources.Load("test") != null || bs.setting.dontLoadAssets)
		{
			resLoaded = (resLoaded2 = true);
			Debug.Log("[VERBOSE] Resources marked as loaded (test asset found or dontLoadAssets set)");
			System.Console.WriteLine("[VERBOSE] Resources marked as loaded (test asset found or dontLoadAssets set)");
		}
		Debug.Log("[VERBOSE] Loader Awake - Calling base.Awake()");
		System.Console.WriteLine("[VERBOSE] Loader Awake - Calling base.Awake()");
		base.Awake();
		Debug.Log("[VERBOSE] Loader Awake - Initialization complete");
		System.Console.WriteLine("[VERBOSE] Loader Awake - Initialization complete");
	}
	...
	public void Start()
	{
		Debug.Log("[VERBOSE] Loader Start - Starting main initialization sequence");
		//IL_025a: Unknown result type (might be due to invalid IL or missing references)
		//IL_0264: Expected O, but got Unknown
		GA.API.Quality.NewEvent("Quality Test", "test");
		Debug.Log("[VERBOSE] Quality test event sent");
		SetOffline(offline: true);
		Debug.Log("[VERBOSE] Offline mode set");
		if (bs.setting.autoHost)
		{
			playerName = "host";
			Debug.Log("[VERBOSE] Auto-host mode enabled, playerName set to 'host'");
		}
		if (bs.setting.autoConnect)
		{
			playerName = "client";
			Debug.Log("[VERBOSE] Auto-connect mode enabled, playerName set to 'client'");
		}
		res.waterMaterial.shader = res.waterMaterialDef;
		Debug.Log("[VERBOSE] Water material shader set");
		if (bs.setting.autoConnect || bs.setting.autoHost)
		{
			...
			Debug.Log("[VERBOSE] Auto mode settings applied - random playerName, carSkin, and avatar");
		}
		if (!UnityEngine.PlayerPrefs.HasKey("playerName") && !UnityEngine.PlayerPrefs.HasKey("firstTime"))
		{
			StartCoroutine(FirstTime());
			Debug.Log("[VERBOSE] First time setup initiated");
		}
		bs.LogEvent("Start");
		Debug.Log("[VERBOSE] Start event logged");
		if (!bs.android && !string.IsNullOrEmpty(Application.absoluteURL))
		{
			try
			{
				...
				if (!string.IsNullOrEmpty(text))
				{
					...
					Debug.Log("[VERBOSE] Referer logged: " + text);
				}
			}
			catch (Exception ex)
			{
				Debug.LogError("[VERBOSE] Error logging referer: " + ex.Message);
			}
		}
		Debug.Log("[VERBOSE] Attempting to download country information");
		bs.Download(bs.mainSite + "scripts/country.php", delegate(string s, bool b)
		{
			if (b)
			{
				bs._Loader.contry = s.ToLower();
				Debug.Log("[VERBOSE] Country set: " + s.ToLower());
			}
			else
			{
				Debug.LogWarning("[VERBOSE] Failed to get country information");
			}
		}, false);
		StartCoroutine(StartLoad());
		Debug.Log("[VERBOSE] StartLoad coroutine initiated");
		MonoBehaviour.print("Init Banner");
		Debug.Log("[VERBOSE] Banner initialization");
		if (showBanner)
		{
			SamsungAd.Instance().Init("xv0d00000002jl", "xv0d00000002jl");
			Debug.Log("[VERBOSE] Samsung ad banner initialized");
		}
		Everyplay.SharedInstance.RecordingStopped += delegate
		{
			everyplayRecorded = true;
			Debug.Log("[VERBOSE] Everyplay recording stopped event handled");
		};
		Everyplay.SharedInstance.UploadDidComplete += delegate
		{
			showBanner = false;
			Debug.Log("[VERBOSE] Everyplay upload completed, banner disabled");
		};
		if (bs.setting.skipLogin || bs.setting.autoHost || bs.setting.autoConnect)
		{
			bs.guest = (loggedIn = true);
			Debug.Log("[VERBOSE] Skip login or auto mode - setting as guest and logged in");
			StartCoroutine(Base.AddMethod(0.1f, (Action)(object)(Action)delegate
			{
				OnLoggedIn(g: true);
			}));
		}
		Debug.Log("[VERBOSE] Attempting to download server time");
		bs.Download(bs.mainSite + "scripts/time.php", delegate(string s, bool b)
		{
			if (b)
			{
				serverTime = DateTime.Parse(s);
				Debug.Log("[VERBOSE] Server time set: " + serverTime);
			}
			else
			{
				Debug.LogWarning("[VERBOSE] Failed to get server time");
			}
			MonoBehaviour.print("Serve rTime " + serverTime);
		}, false);
		GA.API.Quality.NewEvent(bs.setting.version.ToString());
		Debug.Log("[VERBOSE] Version event sent to analytics");
		DownloadUserMaps2(bs._Loader.page);
		Debug.Log("[VERBOSE] User maps download initiated");
		Debug.Log("[VERBOSE] Loader Start - Initialization sequence completed");
	}
	...
	{
		Debug.Log("[VERBOSE] OnLevelWasLoaded called with level: " + level);
		MonoBehaviour.print("OnLevelWasLoaded " + level);
		errors = 0;
		if (level != 0 && showBanner)
		{
			SamsungAd.Instance().DestroyBannerAd();
			Debug.Log("[VERBOSE] Banner ads destroyed for non-zero level");
		}
		else
		{
			Debug.Log("[VERBOSE] Level is 0 or no banner to destroy");
		}
		// Check what scenes are available after loading
		Debug.Log("[VERBOSE] Current active scene: " + UnityEngine.SceneManagement.SceneManager.GetActiveScene().name);
		Debug.Log("[VERBOSE] Scene count in build settings: " + UnityEngine.SceneManagement.SceneManager.sceneCount);
	}
...
public void Update()
	{
		...
		{
			Debug.Log("[VERBOSE] Loader Update - Frame: " + Time.frameCount + ", Time: " + Time.time);
			System.Console.WriteLine("[VERBOSE] Loader Update - Frame: " + Time.frameCount + ", Time: " + Time.time);
		}
